      ...
      <div style="margin-left:15px">
        @if(isset($SectionOne))
          @foreach($SectionOne as $SectionOne)
            <a href="{{ url('本校個人資料保護與管理/'.$SectionOne->id) }}" @if($SectionOne->type==1) target="_blank" @endif><h4>•{{ $SectionOne->title }}</h4></a>
          @endforeach
        @endif
      </div>
    </div>
      ...
      <div style="margin-left:15px">
        @if(isset($SectionTwo))
          @foreach($SectionTwo as $SectionTwo)
            <a href="{{ url('個資資產盤點作業/'.$SectionTwo->id) }}"><h4>•{{ $SectionTwo->title }}</h4></a>
          @endforeach
        @endif
      </div>
    </div>
      ...
      <div style="margin-left:15px">
        @if(isset($SectionThree))
          @foreach($SectionThree as $SectionThree)
            <a href="{{ url('其它/'.$SectionThree->id) }}"><h4>•{{ $SectionThree->title }}</h4></a>
          @endforeach
        @endif
      </div>
    </div>
